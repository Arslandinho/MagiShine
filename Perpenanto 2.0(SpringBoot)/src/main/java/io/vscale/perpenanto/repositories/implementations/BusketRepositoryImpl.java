...
import io.vscale.perpenanto.dto.implementations.BusketDTO;
...
public class BusketRepositoryImpl implements BusketRepository{
    ...
    @Autowired
    public BusketRepositoryImpl(JdbcTemplate template, JdbcTemplateWrapper<Busket> jdbcTemplateWrapper) {
    ...
    public void update(Busket model) {
        ...
        Optional<Busket> tempBusket =
                this.jdbcTemplateWrapper.findItem(BusketRepositoryImpl.this, model.getProfile().getId());
        ...
        tempBusket.ifPresent(busket -> {
            ...
            List<Product> oldProducts = new ArrayList<>(busket.getProducts());

            products.stream()
                    .filter(product -> !oldProducts.contains(product))
                    .forEach(oldProducts::add);

            busket.setProducts(oldProducts);
